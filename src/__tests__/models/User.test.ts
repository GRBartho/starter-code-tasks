...
describe("User Model", () => {
  ...
  beforeAll(async () => {
    ...
    User.init(User.getAttributes(), { ...User.options, sequelize });

    Project.init(Project.getAttributes(), { ...Project.options, sequelize });

    Task.init(Task.getAttributes(), { ...Task.options, sequelize });

    Tag.init(Tag.getAttributes(), { ...Tag.options, sequelize });
  ...
  describe("Validation", () => {
    ...
    it("should validate username length", async () => {
      const wrongUsernameLength = { ...validUserData, username: "" };
      await expect(User.create(wrongUsernameLength)).rejects.toThrow("Validation error: Validation len on username failed");
    });

    it("should validate email format", async () => {
      const invalidEmail = { ...validUserData, email: "notanemail" };
      await expect(User.create(invalidEmail)).rejects.toThrow();
    });

    it("should validate password length", async () => {
      const shortPassword = { ...validUserData, password: "12345" };
      await expect(User.create(shortPassword)).rejects.toThrow("Validation error: Validation len on password failed");

      const longPassword = { ...validUserData, password: "a".repeat(101) };
      await expect(User.create(longPassword)).rejects.toThrow("Validation error: Validation len on password failed");
    });

    it("should validate name lengths", async () => {
      const shortFirstName = { ...validUserData, firstName: "a" };
      await expect(User.create(shortFirstName)).rejects.toThrow("Validation error: Validation len on firstName failed");

      const longFirstName = { ...validUserData, firstName: "a".repeat(51) };
      await expect(User.create(longFirstName)).rejects.toThrow("Validation error: Validation len on firstName failed");
    });
  });
